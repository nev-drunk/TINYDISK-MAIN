...
void OpeDB::init()
{
    ...
    m_db.setDatabaseName("/home/mingli/Desktop/cloud.db");
    if(m_db.open())
    {
        QSqlQuery query;
...
void OpeDB::handleAddFriendAgree(const char *addedName, const char *sourceName)
{
    ...
    if(sourceUserId == -1 || addedUserId == -1)
        return;

    QSqlQuery query;

    // 双向插入
    QString str1 = QString("insert into friend values(%1, %2)").arg(sourceUserId).arg(addedUserId);
    QString str2 = QString("insert into friend values(%1, %2)").arg(addedUserId).arg(sourceUserId);
